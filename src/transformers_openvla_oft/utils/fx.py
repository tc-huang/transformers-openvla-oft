...
class HFTracer(Tracer):
    ...
    def _generate_dummy_input(
        ...
        # rightfully pass certain controlflows (Example: https://github.com/huggingface/transformers/blob/5c8d941d66734811d2ef6f57f15b44f7fb7a98c4/src/transformers_openvla_oft/modeling_attn_mask_utils.py#L162).
    ...
    def trace(
        ...
        Args:
            root (`torch.nn.Module` or  `Callable`):
                ...
                [`~transformers_openvla_oft.PreTrainedModel`], then `dummy_inputs` must be passed, otherwise tracing will fail.
            concrete_args (`Dict[str, Any], *optional*):
                Concrete arguments that should not be treated as Proxies
            dummy_inputs (`Dict[str, Any]`, *optional*):
                The dummy inputs needed to handle data-dependent control-flow if `root` is not a
                [`~transformers_openvla_oft.PreTrainedModel`]. It can also be used when `root` is a
                [`~transformers_openvla_oft.PreTrainedModel`] to specify custom dummy inputs for a subset or all the model inputs.
        ...
        for input_name in input_names:
            ...
            if isinstance(root, self.supported_archs) or type(root).__qualname__.startswith(
                ...
                raise RuntimeError(
                    ...
                    " transformers_openvla_oft.PreTrainedModel."
                )
