...
def run_hp_search_ray(trainer, n_trials: int, direction: str, **kwargs) -> BestRun:
    ...
    def _objective(trial: dict, local_trainer):
        try:
            from transformers_openvla_oft.utils.notebook import NotebookProgressCallback
...
class WandbCallback(TrainerCallback):
    ...
    def __init__(self):
        ...
        if os.getenv("WANDB_LOG_MODEL", "FALSE").upper() in ENV_VARS_TRUE_VALUES.union({"TRUE"}):
            DeprecationWarning(
                ...
                "version 5 of transformers_openvla_oft. Use one of `'end'` or `'checkpoint'` instead."
            )
    ...
    def setup(self, args, state, model, **kwargs):
        ...
        - **WANDB_LOG_MODEL** (`str`, *optional*, defaults to `"false"`):
            ...
            with [`~transformers_openvla_oft.TrainingArguments.load_best_model_at_end`] to upload best model.
...
class DVCLiveCallback(TrainerCallback):
    ...
    def on_train_end(self, args, state, control, **kwargs):
        if self._initialized and state.is_world_process_zero:
            from transformers_openvla_oft.trainer import Trainer
