...
def export_with_optimum(args):
    if is_optimum_available():
        ...
        if parsed_optimum_version < version.parse(MIN_OPTIMUM_VERSION):
            raise RuntimeError(
                f"transformers_openvla_oft.onnx requires optimum >= {MIN_OPTIMUM_VERSION} but {optimum_version} is installed. You "
                "can upgrade optimum by running: pip install -U optimum[exporters]"
            )
    else:
        raise RuntimeError(
            "transformers_openvla_oft.onnx requires optimum to run, you can install the library by running: pip install "
            "optimum[exporters]"
        )
    cmd_line = [
        sys.executable,
    ...
    logger.info(
        ...
        "transformers_openvla_oft.onnx is deprecated, and will be removed in v5. You can find more information here: "
...
def export_with_transformers(args):
    ...
    if model_kind in ENCODER_DECODER_MODELS:
        ...
        warnings.warn(
            "The export was done by transformers_openvla_oft.onnx which is deprecated and will be removed in v5. We recommend"
...
def main():
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Whether to use transformers_openvla_oft.onnx instead of optimum.exporters.onnx to perform the ONNX export. It can be "
...
if __name__ == "__main__":
    logger = logging.get_logger("transformers_openvla_oft.onnx")  # pylint: disable=invalid-name
