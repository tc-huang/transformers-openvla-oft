...
class WhisperAttention(nn.Module):
    ...
    # Copied from transformers_openvla_oft.models.bart.modeling_bart.BartAttention._shape with BART->whisper
    def _shape(self, tensor: torch.Tensor, seq_len: int, bsz: int):
        return tensor.view(bsz, seq_len, self.num_heads, self.head_dim).transpose(1, 2).contiguous()

    # Copied from transformers_openvla_oft.models.bart.modeling_bart.BartAttention.forward with BART->whisper
    def forward(
        self,
...
# Copied from transformers_openvla_oft.models.bart.modeling_bart.BartFlashAttention2 with Bart->Whisper
...
class WhisperSdpaAttention(WhisperAttention):
    # Copied from transformers_openvla_oft.models.bart.modeling_bart.BartSdpaAttention.forward with BART->whisper, Bart->Whisper
    def forward(
        self,
        hidden_states: torch.Tensor,
        key_value_states: Optional[torch.Tensor] = None,
...
# Copied from transformers_openvla_oft.models.mbart.modeling_mbart.MBartEncoderLayer with MBart->Whisper, MBART->WHISPER
...
# Copied from transformers_openvla_oft.models.mbart.modeling_mbart.MBartDecoderLayer with MBart->Whisper, MBART->WHISPER
...
class WhisperModel(WhisperPreTrainedModel):
    ...
    def forward(
        ...
        Example:
         ```python
         >>> import torch
         >>> from transformers_openvla_oft import AutoFeatureExtractor, WhisperModel
...
class WhisperForConditionalGeneration(WhisperGenerationMixin, WhisperPreTrainedModel):
    ...
    def forward(
        ...
        ```python
        >>> import torch
        >>> from transformers_openvla_oft import AutoProcessor, WhisperForConditionalGeneration
        >>> from datasets import load_dataset
...
class WhisperForCausalLM(WhisperPreTrainedModel):
    ...
    def forward(
        ...
        ```python
        >>> from transformers_openvla_oft import WhisperForCausalLM, WhisperForConditionalGeneration, WhisperProcessor
        >>> import torch
        >>> from datasets import load_dataset
...
class WhisperForAudioClassification(WhisperPreTrainedModel):
    ...
    def forward(
        ...
        ```python
        >>> import torch
        >>> from transformers_openvla_oft import AutoFeatureExtractor, WhisperForAudioClassification
        >>> from datasets import load_dataset
