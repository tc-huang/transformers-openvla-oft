...
def _setup_default_tools():
    ...
    main_module = importlib.import_module("transformers_openvla_oft")
...
def get_tool_creation_code(code, toolbox, remote=False):
    code_lines = ["from transformers_openvla_oft import load_tool", ""]
    for name, tool in toolbox.items():
...
class AzureOpenAiAgent(Agent):
    ...
    ```py
    from transformers_openvla_oft import AzureOpenAiAgent
...
class LocalAgent(Agent):
    ...
    ```py
    import torch
    from transformers_openvla_oft import AutoModelForCausalLM, AutoTokenizer, LocalAgent
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, **kwargs):
        ...
        ```py
        import torch
        from transformers_openvla_oft import LocalAgent
