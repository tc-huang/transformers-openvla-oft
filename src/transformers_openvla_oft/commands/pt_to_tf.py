...
class PTtoTFCommand(BaseTransformersCLICommand):
    ...
    def run(self):
        ...
        if self._override_model_class is not None:
            ...
            try:
                pt_class = getattr(import_module("transformers_openvla_oft"), architectures[0])
            except AttributeError:
                raise ValueError(f"Model class {self._override_model_class} not found in transformers_openvla_oft.")
            try:
                tf_class = getattr(import_module("transformers_openvla_oft"), "TF" + architectures[0])
            except AttributeError:
                raise ValueError(f"TF model class TF{self._override_model_class} not found in transformers_openvla_oft.")
        elif architectures is None:  # No architecture defined -- use auto classes
            pt_class = getattr(import_module("transformers_openvla_oft"), "AutoModel")
            tf_class = getattr(import_module("transformers_openvla_oft"), "TFAutoModel")
            ...
            try:
                ...
                raise AttributeError(f"The TensorFlow equivalent of {architectures[0]} doesn't exist in transformers_openvla_oft.")
        ...
        if self._push:
            ...
            commit_descrition = (
                ...
                " CLI](https://github.com/huggingface/transformers/blob/main/src/transformers_openvla_oft/commands/pt_to_tf.py). "
